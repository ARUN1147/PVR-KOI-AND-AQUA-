...
const Orders = () => {
    ...
    const [convertFormData, setConvertFormData] = useState({ totalAmount: '', paidAmount: '', taxPhase: 'Inside TN' });
    ...
    const handleConvertClick = (enq) => {
        ...
        setConvertFormData({ totalAmount: '', paidAmount: '', taxPhase: 'Inside TN' });
    ...
    const handleConvertSubmit = async (e) => {
        ...
        try {
            const orderData = {
                ...
                taxPhase: convertFormData.taxPhase,
                status: 'Quotation',
            ...
            <Modal isOpen={isConvertModalOpen} onClose={() => setIsConvertModalOpen(false)} title="Convert Enquiry to Order">
                ...
                <form onSubmit={handleConvertSubmit} className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-gray-400 uppercase ml-1 italic">Tax Phase</label>
                        <div className="flex gap-4 p-2 bg-gray-50 rounded-xl">
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input 
                                    ...
                                    checked={convertFormData.taxPhase === 'Inside TN'} 
                                    onChange={() => setConvertFormData({ ...convertFormData, taxPhase: 'Inside TN' })} 
                                />
                                <span className={`text-xs font-bold transition-colors ${convertFormData.taxPhase === 'Inside TN' ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-600'}`}>Inside TN</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input 
                                    type="radio" 
                                    name="taxPhase" 
                                    className="accent-primary-600"
                                    checked={convertFormData.taxPhase === 'Outside TN'} 
                                    onChange={() => setConvertFormData({ ...convertFormData, taxPhase: 'Outside TN' })} 
                                />
                                <span className={`text-xs font-bold transition-colors ${convertFormData.taxPhase === 'Outside TN' ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-600'}`}>Outside TN</span>
                            </label>
                        </div>
                    </div>
